...
namespace WindowsObjectScreenPositionControl
{
    public static class ScreenManager
    {
        ...
        /// <summary>
        /// X좌표가 오름차순으로 정렬된 스크린 배열을 반환합니다.
        /// </summary>
        public static Screen[] AsscendingAllScreens
        {
            get
            {
        ...
        public static bool OnlyOneIsScreenConnected
        {
            get
            {
                return 1 == AllScreenLength;
            }
        }
        ...
        public static string ToStringAllScreenBounds(Screen[] screens, bool isAppendLine = false)
        {
            StringBuilder builder = new StringBuilder(256);
            int allScreenLength = screens.Length;
            for (int i = 0; i < allScreenLength; i++)
            {
                Screen screen = screens[i];
                if (screen.Primary)
                {
                    builder.AppendFormat(" {0}. Main {1} ", i + 1, screen.Bounds);
                }
                else
                {
                    builder.AppendFormat(" {0}. Sub {1}", i + 1, screen.Bounds);
                }
