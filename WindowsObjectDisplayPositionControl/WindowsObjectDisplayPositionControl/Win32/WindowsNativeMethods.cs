...
namespace WindowsObjectDisplayPositionControl.Win32
{
    public static class WindowsNativeMethods
    {
        ...
        #region GetParentFormByIntPtrHandle

        private static IntPtr GetParentFormByIntPtrHandle(IntPtr parentHandle, IntPtr childHandle)
        {
            if (parentHandle != IntPtr.Zero)
            {
                childHandle = parentHandle;
                parentHandle = User32.GetParentByIntPtrHandle(parentHandle);
                return GetParentFormByIntPtrHandle(parentHandle, childHandle);
            }
            else
            {
                return childHandle;
        ...
        public static IntPtr GetParentFormByIntPtrHandle(IntPtr hWnd)
        {
            return GetParentFormByIntPtrHandle(User32.GetParentByIntPtrHandle(hWnd), hWnd);
        }
        ...
        public static bool IsParentChildRelationship(IntPtr handleParent, IntPtr handleChild)
        {
            ...
            if (User32.GetParentByIntPtrHandle(handleChild) == handleParent)
            {
                return true;
            }
            return false;
        ...
        {
            return 0 == User32.GetParentByIntHandle(hWnd);
        }
